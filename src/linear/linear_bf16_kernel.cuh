 ...
 *   bias: [N]
 */
__global__ void linear_bf16_kernel_v1(
    ...
    __shared__ cuda_bfloat16 s_b[BK][BN + BPAD];
    ...
    wmma::fragment<wmma::matrix_b, 16, 16, 16, cuda_bfloat16, wmma::row_major> frag_b[2][4];
...
#pragma unroll
                for (int j = 0; j < 8; j++) {
                    if (gmem_m < M && gmem_k + j < K) {
                        ...
                        s_a[smem_m][load_a_smem_k + j] = __float2bfloat16(0.0f);
                    }
                }
            }
        }
...
#pragma unroll
            for (int ni = 0; ni < 8; ni++) {
                ...
                if (gmem_n < N && gmem_k < K) {
                    ...
                    s_b[smem_k][smem_n] = __float2bfloat16(0.0f);
                }
            }
        }
...
#pragma unroll
            for (int idx = lane_id; idx < 256; idx += 32) {
                ...
                if (global_m < M && global_n < N) {
                    ...
                    float b = __bfloat162float(bias[global_n]);
                    C[OFFSET(global_m, global_n, N)] = __float2bfloat16(val + b);
                }
            }
            __syncwarp();
        }
 ...
 *   bias: [N]
 */
__global__ void linear_bf16_kernel_v2(
    cuda_bfloat16 *__restrict__ C,          // [M, N] row-major
    const cuda_bfloat16 *__restrict__ A,    // [M, K] row-major
    const cuda_bfloat16 *__restrict__ B,    // [N, K] row-major (transposed)
    const cuda_bfloat16 *__restrict__ bias, // [N]
...
#pragma unroll
                for (int j = 0; j < 8; j++) {
                    if (gmem_n < N && gmem_k + j < K) {
                        ...
                        s_b[smem_n][load_b_smem_k + j] = __float2bfloat16(0.0f);
                    }
                }
            }
        }
...
#pragma unroll
            for (int idx = lane_id; idx < 256; idx += 32) {
                ...
                s_c_cuda_bfloat16[wid][local_m][local_n] = __float2bfloat16(s_c_float[wid][local_m][local_n] + bias_vals[j]);
            }
            __syncwarp();
            ...
            if (global_m < M && global_n + 7 < N) {
                ...
                FLOAT4(C[OFFSET(global_m, global_n, N)]) = FLOAT4(s_c_cuda_bfloat16[wid][row][col]);
            } else if (global_m < M) {
                // Boundary case: element-wise store
#pragma unroll
                for (int c = 0; c < 8; c++) {
                    if (global_n + c < N) {
                        C[OFFSET(global_m, global_n + c, N)] = s_c_cuda_bfloat16[wid][row][col + c];
                    }
                }
            }
            __syncwarp();
 ...
 *   bias: [N]
 */
__global__ void linear_bf16_kernel_v3(
    cuda_bfloat16 *__restrict__ C,          // [M, N] row-major
    const cuda_bfloat16 *__restrict__ A,    // [M, K] row-major
    const cuda_bfloat16 *__restrict__ B,    // [N, K] row-major (transposed)
    const cuda_bfloat16 *__restrict__ bias, // [N]
...
#pragma unroll
                for (int j = 0; j < 8; j++) {
                    if (gmem_k + j < K) {
                        s_a[smem_m][load_a_smem_k + j] = src_ptr[j];
                    } else {
                        s_a[smem_m][load_a_smem_k + j] = __float2bfloat16(0.0f);
                    }
                }
...
#pragma unroll
        for (int i = 0; i < 4; i++) {
            ...
            uint32_t load_b_smem_addr = __cvta_generic_to_shared(&s_b[smem_n][load_b_smem_k]);
...
#pragma unroll
                for (int j = 0; j < 8; j++) {
                    if (gmem_k + j < K) {
                        s_b[smem_n][load_b_smem_k + j] = src_ptr[j];
                    } else {
                        s_b[smem_n][load_b_smem_k + j] = __float2bfloat16(0.0f);
                    }
                }
 ...
 *   bias: [N]
 */
__global__ void linear_bf16_kernel_v4(
    cuda_bfloat16 *__restrict__ C,          // [M, N] row-major
    const cuda_bfloat16 *__restrict__ A,    // [M, K] row-major
    const cuda_bfloat16 *__restrict__ B,    // [N, K] row-major (transposed)
    const cuda_bfloat16 *__restrict__ bias, // [N]
    ...
    // __shared__ cuda_bfloat16 s_a[BM][BK + APAD];
    // __shared__ cuda_bfloat16 s_b[BN][BK + BPAD];
    extern __shared__ cuda_bfloat16 smem[];
    cuda_bfloat16 *s_a = smem;
