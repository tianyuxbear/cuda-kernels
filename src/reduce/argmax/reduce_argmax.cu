...
int main(int argc, char *argv[]) {
  ...
  unsigned long long h_packed_res =
      pack(-std::numeric_limits<float>::infinity(), UINT32_MAX);
  ...
  CUDA_CHECK(cudaMemcpy(d_packed_res, &h_packed_res, sizeof(unsigned long long),
  ...
  CUDA_CHECK(cudaMemcpy(&h_packed_res, d_packed_res, sizeof(size_t),
