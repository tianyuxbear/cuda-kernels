...
template <typename T> __device__ inline float to_float_val(const T &x) {
  if constexpr (std::is_same_v<T, float>) {
    return x;
  } else if constexpr (std::is_same_v<T, half>) {
...
#pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
    ...
    if (other_v > v ||
        (other_v == v && other_idx != UINT32_MAX && other_idx < idx)) {
      v = other_v;
      idx = other_idx;
    }
  }
...
#pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
      ...
      if (other_v > block_v || (other_v == block_v && other_idx != UINT32_MAX &&
                                other_idx < block_idx)) {
        block_v = other_v;
        block_idx = other_idx;
      }
