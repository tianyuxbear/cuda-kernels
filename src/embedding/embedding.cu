...
int main(int argc, char *argv[]) {
    ...
    // Copy data to device
    CUDA_CHECK(cudaMemcpy(d_weight, h_weight.data(), M * K * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_output, h_output.data(), N * K * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_index, h_index.data(), N * sizeof(int), cudaMemcpyHostToDevice));
    ...
    float memory_usage_mb = ((M * K + N * K) * sizeof(float) + N * sizeof(int)) / (1024.0 * 1024.0);
    ...
    CUDA_CHECK(cudaMemcpy(h_output_gpu.data(), d_output, N * K * sizeof(float), cudaMemcpyDeviceToHost));

    // CPU reference calculation
    embedding_cpu(h_output, h_weight, h_index, K);

    // Verify results
    bool verification_passed = verify_results(h_output.data(), h_output_gpu.data(), N * K);
