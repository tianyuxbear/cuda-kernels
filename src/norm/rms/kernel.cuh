...
__global__ void rmsnorm_kernel_smem(T *output, const T *input, const T *weight,
  ...
  for (size_t i = tid; i < len; i += blockDim.x)
    if constexpr (std::is_same_v<T, float>) {
      ...
      row_out[i] = __float2half(__half2float(row_in[i]) *
                                __half2float(weight[i]) * inv_std);
    } else if constexpr (std::is_same_v<T, cuda_bfloat16>) {
      row_out[i] = __float2bfloat16(__bfloat162float(row_in[i]) *
                                    __bfloat162float(weight[i]) * inv_std);
    }
}
