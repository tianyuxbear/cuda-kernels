...
int main(int argc, char *argv[]) {
    ...
    CUDA_CHECK(cudaMemcpy(d_weight, h_weight.data(), len * sizeof(float), cudaMemcpyHostToDevice));
    ...
    for (int i = 0; i < WARM_UP_ITERS; ++i) {
        test_kernel<<<gridDim, blockDim, shared_mem_size>>>(d_output, d_input, d_weight, EPSILON, len);
    ...
    float memory_usage_mb = (2.0 * nbytes + len * sizeof(float)) / (1024.0 * 1024.0);
