...
void rmsnorm_cpu(std::vector<float> &output, const std::vector<float> &input,
                 const std::vector<float> &weight, const float eps,
                 const size_t nrow, const size_t len) {
  if (len == 0) {
  ...
  for (size_t r = 0; r < nrow; ++r) {
    ...
    for (size_t i = 0; i < len; ++i) {
      float normalized = input[base + i] * weight[i] * inv_std;
...
#ifdef TEST1
#define test_kernel rmsnorm_kernel_smem
#define config_name "rmsnorm_smem"
#endif

#ifdef TEST2
#define test_kernel rmsnorm_kernel_warp
#define config_name "rmsnorm_warp"
#endif

#ifdef TEST3
#define test_kernel rmsnorm_kernel_smem_vec
#define config_name "rmsnorm_smem_vec"
#endif
...
int main(int argc, char *argv[]) {
  ...
  CUDA_CHECK(cudaMalloc(&d_weight, len * sizeof(float)));

  // Copy data to device
  CUDA_CHECK(
      cudaMemcpy(d_input, h_input.data(), nbytes, cudaMemcpyHostToDevice));
  CUDA_CHECK(
      cudaMemcpy(d_output, h_output.data(), nbytes, cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(d_weight, h_weight.data(), len * sizeof(float),
  ...
  for (int i = 0; i < WARM_UP_ITERS; ++i) {
    test_kernel<<<gridDim, blockDim, shared_mem_size>>>(d_output, d_input,
                                                        d_weight, EPSILON, len);
  ...
  float memory_usage_mb =
      (2.0 * nbytes + len * sizeof(float)) / (1024.0 * 1024.0);

  size_t bytes_accessed = 3 * nbytes + len * sizeof(float);
  float throughput_gbs = (bytes_accessed / (avg_kernel_time_ms / 1000.0)) / 1e9;

  size_t flops = 4 * nrow * len;
  float compute_perf_gflops = (flops / (avg_kernel_time_ms / 1000.0)) / 1e9;

  // Copy the result back to host
  CUDA_CHECK(cudaMemcpy(h_output_gpu.data(), d_output, nbytes,
                        cudaMemcpyDeviceToHost));

  // CPU reference calculation
  rmsnorm_cpu(h_output, h_input, h_weight, EPSILON, nrow, len);

  // Verify results
  bool verification_passed =
      verify_results(h_output.data(), h_output_gpu.data(), nrow * len, 1e-5f);
